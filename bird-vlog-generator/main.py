...
from config import OUTPUT_DIR, BGM_DIR
...
def get_random_bgm():
    ...
    for ext in ('.mp3', '.wav', '.m4a'):
        bgms.extend(glob.glob(os.path.join(BGM_DIR, f"*{ext}")))
    return bgms[0] if bgms else None
...
def process_single_video(
    ...
    if bgm_path:
        print(f"🎵 发现 BGM: {os.path.basename(bgm_path)}")
    ...
    if mode == "slideshow":
        ...
        compose_video(input_video, audio_path, output_path, 
                      subtitle_file=srt_path, bgm_path=bgm_path)
